...
def fetch(browser, cfg, filters, dates) -> set[tuple[str, str, str]]:
    ...
    try:
        ...
        _click(page, "公共施設予約メニュー")
        _click(page, re.compile("空き照会・予約の申込"))

        # 施設検索: 「区立運動場等」を選んで次へ
        page.get_by_text("区立運動場等", exact=False).first.click()
        page.wait_for_timeout(1000)
        _click(page, re.compile("次へ"))

        # 日時選択: 1ヶ月・土日祝（各クリックでポストバックが走る）
        for name in ["1ヶ月", "土", "日", "祝"]:
            _click(page, name, exact=True)
        _click(page, re.compile("次へ"))

        # 施設別空き状況（今月）→ 次の期間（翌月）
        for period in range(2):
            month_hits = _parse_month_grid(page, cfg["facilities"], date_set)
            if month_hits:
                slot_results = _drill_time_slots(page, month_hits, filters)
            ...
            if period == 0:
                try:
                    _click(page, re.compile("次の期間"))
                except Exception:  # noqa: BLE001
                    break
        return results
    except Exception:
        dump_debug(page, "chuo_error")
        raise
    finally:
        ctx.close()


def _click(page, name, exact=False):
    page.get_by_role("button", name=name, exact=exact).first.click()
    page.wait_for_load_state("domcontentloaded")
    page.wait_for_timeout(1500)


def _parse_month_grid(page, facilities, date_set):
    """施設別空き状況の表から (施設, 日付, ○|△) を抽出"""
    body = page.inner_text("body")
    ym = re.search(r"(\d{4})年(\d{1,2})月", body)
    if not ym:
        dump_debug(page, "chuo_no_month")
        return []
    ...
    for i, row in enumerate(data):
        if sum(1 for c in row if re.fullmatch(r"\d{1,2}\s*[月火水木金土日祝]", c)) >= 3:
            ...
            break
    if header is None:
        dump_debug(page, "chuo_no_header")
        return []
    ...
    for row in data[hidx + 1:]:
        ...
        if fac not in facilities:
            continue
        # 行のセル位置とヘッダ位置は施設名・定員列分ずれるので、日付列の個数で合わせる
...
def _drill_time_slots(page, month_hits, filters):
    """○/△セルを選択して時間帯別空き状況へ進み、時間帯を取得。
    失敗したら None を返す（呼び出し側で日単位にフォールバック）"""
    try:
        # 対象施設の行にある ○/△ リンクを全てクリックして「選択」状態にする
        # （表示曜日を土日祝に絞ってあるので、表に出ている日は全て監視対象日）
        selected = 0
        for fac in sorted({f for f, _, _ in month_hits}):
            row = page.locator("tr", has_text=fac).last
        ...
        # 時間帯別空き状況の解析
        slots = _parse_time_page(page, filters)
        # 元の画面（施設別空き状況）へ戻る
        _click(page, re.compile("戻る"))
        return slots
    except Exception as e:  # noqa: BLE001
        print(f"[chuo] 時間帯別の取得に失敗: {e}")
        dump_debug(page, "chuo_timeslot_error")
        return None


def _parse_time_page(page, filters):
    """時間帯別空き状況ページから (施設, 日付, 時間帯) を抽出"""
    results = set()
    ...
    for t in tables:
        ...
        if not rows:
            continue
        # ヘッダから時間帯ラベルを取得（例 "９時～１１時" "9:00～11:00"）
        header = rows[0]
        time_cols = {}
        for i, c in enumerate(header):
            c2 = _z2h(c)
